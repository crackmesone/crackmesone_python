...
def test_first_blood_discord_notification_uses_public_channel(app):
    from app.services import discord
    ...
    with patch.object(discord, 'send_to_webhook', return_value=True) as send:
        assert discord.notify_first_blood('bob', 'Challenge', 'abc', 300) is True
        assert send.call_args.args[0].endswith('/public')
        public_embed = send.call_args.kwargs['embed']
        assert public_embed['title'] == 'First Blood'
